...
class complex_number(math_block):
         ...
         def __mul__(self, other):
                  ...
                  if isinstance(other, complex_number):
                           ...
                           if self.form == "rect":
                                    return complex_number(self.real*other, self.imag*other)
                           else:
                                    return complex_number.from_polar(self.modulus*other, self.angle)
         ...
         def latex(self, explicit=False, show_plus=False):
                  if self.form == "rect":
                           ...
                           out = "{}{}i".format(real_latex, imag_latex)
                  else:
                           out = "{0}(\cos{1}+i\sin{1})".format(self.modulus, self.angle)

                  if self.sign:
                           return "+{}".format(out) if show_plus else out
